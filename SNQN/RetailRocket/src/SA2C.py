...
def parse_args():
    ...
    parser.add_argument(
        "--clip", type=float, default=0.0, help="clip value for advantage"
    )
...
class QNetwork:
    def __init__(
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.len_state = tf.compat.v1.placeholder(
                tf.int32, [None]
            ...
            if self.model == "Caser":
                mask = tf.expand_dims(
                    tf.cast(tf.not_equal(self.inputs, item_num), dtype=tf.float32), -1
                )
                ...
                with tf.compat.v1.name_scope("dropout"):
                    self.states_hidden = tf.compat.v1.layers.dropout(
                        self.final,
            ...
            if self.model == "SASRec":
                pos_emb = tf.nn.embedding_lookup(
                    params=self.all_embeddings["pos_embeddings"],
                    ids=tf.tile(
                        tf.expand_dims(tf.range(tf.shape(input=self.inputs)[1]), 0),
            ...
            self.targetQs_selector = tf.compat.v1.placeholder(
            ...
            self.targetQ_current_ = tf.compat.v1.placeholder(
                tf.float32, [None, item_num]
            )
            self.targetQ_current_selector = tf.compat.v1.placeholder(
                tf.float32, [None, item_num]
            )  # used for select best action for double q learning

            # calculate propensity score
            ce_logits = tf.stop_gradient(self.output2)
            target_prob = indexing_ops.batched_index(
                tf.nn.softmax(ce_logits), self.actions
            )
            self.behavior_prob = tf.compat.v1.placeholder(
                tf.float32, [None], name="behavior_prob"
            )
            ...
            ce_loss_pre = tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.actions, logits=self.output2
            )
            ce_loss_post = tf.multiply(self.ips, ce_loss_pre)

            q_indexed_positive = tf.stop_gradient(
                indexing_ops.batched_index(self.output1, self.actions)
            )
            q_indexed_negative = 0
            for i in range(self.neg):
                negative = tf.gather(self.negative_actions, i, axis=1)
                q_indexed_negative += tf.stop_gradient(
                    indexing_ops.batched_index(self.output1, negative)
                )
            q_indexed_avg = tf.divide(
                (q_indexed_negative + q_indexed_positive), 1 + self.neg
            )
            ...
            self.loss1 = tf.reduce_mean(
                input_tensor=qloss_positive + qloss_negative + ce_loss_pre
            )
            self.opt1 = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(
                self.loss1
            )

            self.loss2 = tf.reduce_mean(
                input_tensor=self.weight * (qloss_positive + qloss_negative)
                + ce_loss_post
            )
        ...
        if self.pretrain == False:
            with tf.compat.v1.variable_scope(self.name):
                state_embeddings = tf.Variable(
                    tf.random.normal([self.item_num + 1, self.hidden_size], 0.0, 0.01),
